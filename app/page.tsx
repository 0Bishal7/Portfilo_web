...
const academicaeScreens = [
  { src: "/projects/academicae/assignment-builder.png", width: 2898, height: 1758, title: "Multilingual assignment builder", copy: "A guided workflow for class, section, curriculum, language, chapter, and assignment configuration." },
  { src: "/projects/academicae/teacher-dashboard.png", width: 2932, height: 1747, title: "Teacher operations dashboard", copy: "Classes, attendance, assignments, timelines, and academic activity brought into one workspace." },
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="brief-section shell" id="about">
      ...
      <div className="about-panel"><div><p className="about-lead">I am a Python Backend and Full-Stack Developer with 3+ years of experience building scalable APIs, AI-driven education platforms, automation systems, and real-time trading applications.</p><p>My work focuses on backend architecture, FastAPI, Django, REST APIs, WebSockets, PostgreSQL, authentication, performance optimization, and production-ready software development.</p><p><a href="https://kuberav.com/" target="_blank" rel="noreferrer">Kuberav</a> is my independent product—designed, architected, and built from scratch. <a href="https://hello-teacher.ai/" target="_blank" rel="noreferrer">Hello Teacher AI</a> and <a href="https://aps.academicae.com/" target="_blank" rel="noreferrer">Academicae Schools</a> are company products where I made significant engineering contributions across backend APIs, authentication, academic workflows, automation, and performance.</p></div><div className="about-values"><span><b>01</b>Design for reliability</span><span><b>02</b>Measure before optimizing</span><span><b>03</b>Keep complexity understandable</span><span><b>04</b>Ship meaningful outcomes</span></div></div>
    </section>
    ...
    <section className="brief-section section-tint" id="experience"><div className="shell">
      ...
      <div className="experience-timeline">{experience.map((item) => <motion.article className="role-card" key={`${item.role}-${item.period}`} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .2 }}>
        <div className="role-meta"><span>{item.period}</span><h3>{item.role}</h3><b>{item.company}</b><small>{item.location}</small></div><ul>{item.points.map(point => <li key={point}>{point}</li>)}</ul>
      </motion.article>)}</div>
    </div></section>
    ...
    <section className="brief-section shell" id="projects">
      ...
      <div className="featured-grid">{orderedProjects.map((project, index) => <motion.article className={`featured-card project-tone-${index + 1}`} key={project.slug} whileHover={{ y: -6 }}>
        ...
        <span className="project-eyebrow">{project.eyebrow}</span><span className="project-ownership">{project.ownership}</span><h3>{project.name}</h3><p>{project.summary}</p><div className="tag-list">{project.tags.slice(0, 4).map(tag => <span key={tag}>{tag}</span>)}</div><div className="card-actions"><a href={project.liveUrl} target="_blank" rel="noreferrer">Visit Website <Arrow/></a><Link href={`/projects/${project.slug}`}>Case Study <Arrow/></Link></div>
      </motion.article>)}</div>
    </section>

    <section className="brief-section kuberav-case" id="case-study"><div className="shell">
      <SectionHeading eyebrow="KUBERAV · INDEPENDENT PRODUCT" title="Designed and built from scratch." copy="I independently designed the architecture and built Kuberav end to end—from backend services and broker integrations to real-time execution, automation, and risk controls."/>
